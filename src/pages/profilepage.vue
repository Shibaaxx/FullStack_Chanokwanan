      ...
      <ul>
        <li v-for="task in tasks" :key="task.id" class="flex items-center justify-between border-b py-2">
          <div class="flex items-center gap-2">
            <input type="checkbox" v-model="task.completed" @change="updateTaskStatus(
              task)" />
            <span :class="{ 'line-through text-gray-400': task.completed}">{{ task.title
              }}</span>
          </div>
...
<script>
  ...
  export default {
    data() {
      return {
    user: {
      name: 'ชนกวนันท์ ประสมศรี',
      role: 'เจ้าหน้าที่',
      avatar: 'avartar.png'
    },
  newTaskTitle: '',
  tasks: [
  {id: 1, title: 'ท่ารายงานประจําเดือน',completed: false },
